...
def train_one_epoch(model, loader, optimizer, device, epoch, total_epochs, swd_criterion, recon_criterion, use_amp=False, do_wandb=False, queue_log_interval=20, gan=None):
    ...
    with tqdm(total=len(loader.dataset), desc=f'Epoch {epoch}/{total_epochs}', unit='img') as pbar:
        for batch_idx, batch in enumerate(loader):
            ...
            running["cont_reg_loss"] += cont_reg_loss.item()
...
def validate_one_epoch(model, loader, device, swd_criterion, dataset_name, recon_criterion, use_amp=False, fid_bundle=None):
    model.eval()
    running = {
        "loss": 0.0,
        "recon_loss": 0.0,
...
def log_metrics(epoch, train_metrics, val_metrics, valset, model, args):
    ...
    wandb.log({
        ...
        "Train/Content Regularization Loss": train_metrics["cont_reg_loss"],
        ...
        "Val/Content Regularization Loss": val_metrics["cont_reg_loss"],
